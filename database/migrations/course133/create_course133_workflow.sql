        ...
        IF OBJECT_ID(N'[course133].[app_role]', N'U') IS NOT NULL THROW 50002          ,
            ...
            CREATE TABLE [course133].[course_request]
                (
                    ...
                    CONSTRAINT [CK_request_status] CHECK ([status] IN ('DRAFT','PENDING_SIGNED_DOCUMENT','UNDER_OFFICER_REVIEW','RETURNED_FOR_REVISION','PENDING_APPROVAL','REJECTED','PENDING_COURSE_ID','COURSE_ID_RECORDED'))                                                                            ,
                    CONSTRAINT [CK_request_students] CHECK ([expected_students] > 0)                                                                                                                                                                                                                        ,
                    CONSTRAINT [CK_request_learning_period] CHECK ( ([learning_mode] = N'แบบเรียนรู้ตามอัธยาศัยตลอดเวลา' AND [starts_on] IS NULL AND [ends_on] IS NULL) OR ([learning_mode] = N'แบบกำหนดช่วงเวลาเรียน' AND [starts_on] IS NOT NULL AND [ends_on] IS NOT NULL AND [ends_on] >= [starts_on]) ),
            ...
            CREATE TABLE [course133].[course_document]
                (
                    ...
                    CONSTRAINT [CK_document_type] CHECK ([document_type] IN ('GENERATED_FORM','SIGNED_FORM','ADDITIONAL_DOCUMENT')),
